...
## Effective Schema Graph and Variant Semantics

Every SchemaNode in the analysis graph has a corresponding EffectiveSchemaNode.  
EffectiveSchemaNodes represent the resolved semantic meaning of the original schema after all applicators (allOf, oneOf, anyOf) and atomic constraints are incorporated.

### Node-backed vs Branch-only Variants
- **Node-backed variants** correspond directly to real SchemaNodes written in the OpenAPI document.
- **Branch-only variants** occur when branch enumeration produces valid branches that do not correspond to any explicit schema object. These remain anonymous Schemas (not nodes).

### MultiType Effective Nodes
A schema is considered MultiType only when valid variants belong to different base types (integer, string, object…).  
Multiple oneOf structures do **not** imply MultiType unless branches differ in base type.

### Variant Deduplication and Subsumption
For each SchemaNode:
1. Collect all validated variants (node-backed + anonymous).
2. Group variants by base type.
3. Identify **duplicates**:
   - Variants with identical constraint profiles (ranges, required properties, enums, etc.).
4. Identify **subsumed** variants:
   - Variant A is subsumed by B if all constraints of A are stricter or narrower than those of B.
5. These findings may be used for:
   - Diagnostics,
   - Generator simplification,
   - Reducing redundant variant structures.

This step never creates new nodes; it only refines the effective interpretation.
