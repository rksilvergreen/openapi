...
class CallbackDocNode extends DocNode with DocInternalNode, Referencable {
  ...
  void createChildNodes() {
    createNode<PathsMapDocNode>(jsonKey: 'expressions', required: true);
  }

  @override
  void createContent() {
    expressions = $to.to<PathsMapDocNode>('expressions')!;
  ...
  String? _deriveFromCallbackKey() {
    // Get the callback key from the CallbacksMapDocNode edge
    // Path: callback ← callbacksMap ← operation
    final edge = trueParentEdge<CallbacksMapDocNode>();
    if (edge != null) {
      final callbacksMapDocNode = edge.from as CallbacksMapDocNode;
  ...
  String _generateHashFallback() {
    ...
    if (edge != null) {
      ...
      if (callbacksMapDocNode.trueParentEdge<ComponentsDocNode>('callbacks') != null) {
        ...
        // Get the operation
        final operation = callbacksMapDocNode.trueParent<OperationDocNode>('callbacks');
        if (operation != null) {
