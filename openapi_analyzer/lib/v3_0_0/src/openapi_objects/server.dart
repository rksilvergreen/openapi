...
class ServerNode extends Node with InternalNode implements Server {
  ...
  String? _deriveFromContext() {
    ...
    final serverListNode = trueParent<ServerListNode>('servers');
    if (serverListNode == null) return null;

    // Get the index of this server in the list
    final servers = serverListNode.toList();
    int? index;
    ...
    final openApiDoc = serverListNode.trueParent<OpenApiDocumentNode>('servers');
    if (openApiDoc != null) {
      ownerName = openApiDoc.$name;
    } else {
      // Check if it's from a PathItem
      final pathItem = serverListNode.trueParent<PathItemNode>('servers');
      if (pathItem != null) {
        ...
        final operation = serverListNode.trueParent<OperationNode>('servers');
        if (operation != null) {
